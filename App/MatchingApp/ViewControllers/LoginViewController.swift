...
//  MatchingApp
//
//  Created by Joseph Wang on 2021/8/13.
//
...
class LoginViewController: UIViewController {
    ...
    private let dontHaveAccountButton = UIButton(type: .system).createAboutAccountButton(text: "kk")
    ...
    private func setBindings(){
        ...
        loginButton.rx.tap
            .asDriver()
            .drive{[weak self] _ in self?.login()
            }
            .disposed(by: disposeBag)
    }
    ...
    private func login() {
        ...
        HUD.show(.progress)
        Auth.loginWithFireAuth(email: email, password: password) {
            (success) in
            HUD.hide()
            if success {
